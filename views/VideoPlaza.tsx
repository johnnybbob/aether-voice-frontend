    ...
    <div className="min-h-screen bg-slate-950 pt-24 px-6 md:px-10 pb-20 animate-in fade-in duration-300">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-3xl font-bold text-slate-100">衍生视频广场</h1>
        </div>

        {/* Hero Section */}
        <div className="relative w-full h-96 rounded-3xl overflow-hidden mb-12 shadow-2xl shadow-indigo-900/20 group cursor-pointer border border-slate-800">
           ...
           <div className="absolute bottom-0 left-0 p-10 max-w-2xl text-white z-10">
              <span className="inline-block px-3 py-1 bg-rose-600 rounded-full text-xs font-bold mb-4">今日推荐</span>
        ...
        <div>
           <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-2">
                 {['全部', '剧情', '音乐', '搞笑', '游戏'].map(tag => (
                    <button key={tag} className="px-4 py-1.5 rounded-full text-xs font-bold bg-slate-900 border border-slate-800 text-slate-400 hover:bg-slate-800 hover:text-indigo-400 transition-colors">
                       {tag}
                    </button>
                 ))}
              </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {VIDEO_PLAZA_DATA.map((video) => {
                 ...
                 return (
                    <div key={video.id} className="bg-slate-900 rounded-2xl overflow-hidden border border-slate-800 hover:shadow-xl hover:shadow-indigo-500/10 transition-all group flex flex-col">
                       {/* Video Thumbnail */}
                       <div className="h-48 w-full relative flex items-center justify-center cursor-pointer overflow-hidden">
                          ...
                          <div className="absolute w-12 h-12 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white border border-white/30 group-hover:scale-110 transition-transform">
                       ...
                       <div className="p-4 flex-1 flex flex-col">
                          <h3 className="font-bold text-slate-200 text-base mb-2 line-clamp-2 leading-snug group-hover:text-indigo-400 transition-colors">
                             {video.title}
                          </h3>
                          ...
                          {relatedAgent && (
                             <div className="mt-auto pt-4 border-t border-slate-800 flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                   <div className="text-[10px] font-bold text-slate-500">关联角色</div>
                                   <div 
                                      className="flex items-center gap-2 p-1 pr-3 rounded-full bg-slate-800 hover:bg-indigo-900/30 cursor-pointer transition-colors border border-slate-700 hover:border-indigo-800"
                                      onClick={(e) => {
                                   ...
                                   >
                                      ...
                                      <span className="text-xs font-bold text-slate-300">{relatedAgent.name}</span>
